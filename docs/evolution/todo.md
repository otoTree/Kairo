...
- [x] 编写集成测试：Agent 通过 IPC 与 Python REPL 交互
...
- [x] 编写集成测试：Skill 进程无法操作其他 Skill 的进程
...
- [x] 添加 `namespace` 参数到 `add()` 和 `recall()`，实现多 Agent 记忆隔离
- [x] 实现 `share(fromNamespace, toNamespace, memoryId)` 方法
- [x] 优化 GC：使用 LMDB 二级索引按 layer 过滤，避免全表扫描
- [x] 添加 `export(format)` 和 `import(data)` 方法
- [ ] 编写性能测试：10000 条记忆的 GC 耗时 < 1s

### 2.2 盲盒编排落地 ✅

- [x] 完善 `Vault.resolveWithToken` 链路，确保所有工具调用都通过 token
- [x] 实现 Kernel Fingerprint：在 `BinaryRunner` 中计算二进制 hash 并注入环境变量
- [x] 在 Vault 中添加审计日志（访问者、时间、句柄 ID）
- [x] 实现凭证轮换通知：Vault 变更时发布 `kairo.vault.rotated` 事件
- [x] 编写安全测试：无 token 的进程无法解析 vault handle

### 2.3 Agent 协作模型 ✅

- [x] 在 `agent.plugin.ts` 中实现 `delegateTask(parentId, childId, task)` 方法
- [x] 在 Agent Runtime 中添加 `kairo.agent.{id}.task` 事件订阅
- [x] 实现能力声明：Agent 启动时发布 `kairo.agent.capability` 事件
- [x] 在路由层利用能力声明进行智能分发
- [x] 编写集成测试：两个 Agent 协作完成文件处理任务
...
- [x] 实现文件系统挂载（/proc, /sys, /dev, /tmp）
- [x] 实现服务依赖图解析和启动编排
- [x] 添加服务健康检查（定期 ping）
- [x] 实现服务自动重启（最多 3 次，间隔递增）
- [x] 编写集成测试：ServiceManager 拓扑排序、健康检查、自动重启

### 3.2 设备管理生命周期 ✅

- [x] 在 `DeviceRegistry` 中实现 `claim` / `release` 的并发控制（互斥锁）
- [x] 实现热插拔事件 → Agent 通知链路
- [x] 为 Agent 注册 `kairo_device_claim` / `kairo_device_release` 系统工具
- [x] 实现驱动抽象层统一接口（Serial/GPIO/Camera）
- [x] 编写集成测试：模拟设备热插拔

### 3.3 UI 合成器完善 ✅

- [x] 在 `KairoDisplay.zig` 中实现 `user_action` 事件发送
- [x] 在 `compositor.plugin.ts` 中实现信号路由（KDP → EventBus）
- [x] 实现 UI diff 算法（对比新旧 UI 树，仅更新变化节点）
- [x] 扩展 UI 组件集：List、Image、Chart、Modal
- [x] 编写端到端测试：用户点击按钮 → Agent 响应

### 3.4 D-Bus 集成 ✅

- [x] 在 Zig Init 进程中启动 D-Bus daemon
- [x] 实现 D-Bus → EventBus 桥接
- [x] 通过 D-Bus 控制 systemd 服务（start/stop/restart）
- [x] 集成 NetworkManager（网络配置查询和修改）
