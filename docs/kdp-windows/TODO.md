...
> 基于现有实现状态和规范文档，按优先级排列。
> 状态标记：⬜ 未开始 | 🔲 进行中 | ✅ 已完成

---

## Phase 1：KDP 渲染器增强（Zig 层）
...
Phase 1（渲染器增强）
  ├── Phase 2（品牌窗口）← 最先可交付
  ├── Phase 5（设计基础设施）← 与 Phase 2 并行
  └── Phase 6（协议扩展）
       ├── Phase 3（终端）← 依赖键盘事件
       └── Phase 4（文件管理器）← 依赖图片/输入/滚动
```
