...
export type LinuxNetworkBridgeContext = {
...
export type LinuxSandboxParams = {
  command: string
...
/**
 * Probe whether bubblewrap can run with required namespace/mount features
 * in the current environment. This runs a minimal bwrap command and returns
 * true on success. Many constrained container environments will fail here.
 */
export function canUseBwrap(): boolean {
  try {
    const result = spawnSync('bwrap', ['--unshare-pid', '--dev', '/dev', '--', '/bin/true'], {
      stdio: 'ignore',
      timeout: 2000,
    })
...
/**
 * Fallback wrapper for Linux when bubblewrap is unavailable.
 *
 * Provides a "no-bwrap" isolation mode that:
 * - Starts commands with proxy environment variables set (HTTP/HTTPS/SOCKS)
 * - Applies seccomp filter via apply-seccomp (if supported and allowAllUnixSockets is false)
 *
 * Limitations:
 * - No PID namespace or filesystem mount isolation
...
function wrapCommandWithSandboxLinuxNoBwrap(
  ...
  if (!allowAllUnixSockets) {
    ...
    if (seccompFilterPath && applySeccompBinary) {
      const wrapped = shellquote.quote([
        'env',
        ...proxyEnv,
        applySeccompBinary,
        seccompFilterPath,
...
export async function wrapCommandWithSandboxLinux(
  ...
  try {
    // If network restrictions are requested but Linux bridge socket paths
    // are not available (e.g., bubblewrap not usable in this environment),
    // fall back to a no-bwrap isolation mode that sets proxy env vars and
    // applies seccomp (if available).
